...
// usage:
//   ./v545_synchro -m min_position -M max_position -D delta_position -u usec_pause -f excitation_freq -1 -d -r -P target_position -V target_velocity -S sbits
// defaults:
//   min_position = -90.0
//   max_position = 90
//   usec_pause = 200000 (0.2 sec)
//   excitation_freq = 400.0
//   delta_position = 10.0 deg
//   debug_flag = on
//   reset_flag = off
//   one_pass_flag = on
// override parameters:
//   target_position = 45.0
//   target_velocity = 22.5 deg/sec

// using function block channel 0 for synchro output
// using function block channel 1 for synchro input
// using override block 12
// using DDS channel 0

// This program is designed to demonstrate a synchro output (using the DDS for excitation) going out P1, looped back into P2 for synchro read.
// The output uses function block 0 with coil 0 for excitation R1:R2, coil 1 for V(S3:S1), coil 2 for V(S2:S3), and coil 3 for V(S1:S2) on the P1 connector.
// The input uses function block 1 coil 12 for excitation R1:R2, coil 13 for V(S3:S1), coil 14 for V(S2:S3), and coil 15 for V(S1:S2) on the P2 connector.
...
#define SYNCHRO_OUTPUT_FB_CHNL 0
#define SYNCHRO_INPUT_FB_CHNL 1
...
int main(int argc, char *argv[])
{
  ...
  char c;
  char usage[] = "Usage: ./v545_synchro -m min_position -M max_position -D delta_position -u usec_pause -f excitation_freq -1 -d -r -P target_position -V target_velocity -S sbits\n";
  int one_pass_flag = 1;
  int loop_flag = 1;
  char ans[8];
  ...
  if (v545_config_synchro_output(pb, SYNCHRO_OUTPUT_FB_CHNL, DDS_CHNL, excitation_freq, 0, 1, 2, 3, 0, 0, 0, min_position, max_position, min_position, sbits, signed_flag) == -1)
    {
      printf("ERROR %s/%s: v545_config_synchro_output() failure\n", whoami, __FUNCTION__);
      exit(1);
    }
  ...
  //                                         |             |   |   |   |
  //                                         V             V   V   V   V
  if (v545_config_synchro_input(pb, SYNCHRO_INPUT_FB_CHNL, 12, 13, 14, 15, signed_flag) == -1)
    {
      printf("ERROR %s/%s: v545_config_synchro_input() failure\n", whoami, __FUNCTION__);
      exit(1);
    }

  // setup min counts on synchro read

  write_position = min_position;
  
  if (v545_write_synchro_resolver(pb, SYNCHRO_OUTPUT_FB_CHNL, min_position, max_position, write_position, signed_flag) == -1)
    {
  ...
  v545_read_synchro_resolver_position_counts(pb, SYNCHRO_INPUT_FB_CHNL, &min_counts, signed_flag);

  // setup max counts on synchro read
  ...
  v545_read_synchro_resolver_position_counts(pb, SYNCHRO_INPUT_FB_CHNL, &max_counts, signed_flag);
  ...
  // setup software trigger for override function for override block OVERRIDE_BLOCK_CHNL for function block 0, set at 45 degrees at 1 degree per second (with no min/max counts)
  ...
  if (v545_config_override_position_velocity(pb, OVERRIDE_BLOCK_CHNL, SYNCHRO_OUTPUT_FB_CHNL, target_position, target_velocity, min_counts, max_counts, min_position, max_position, signed_flag) == -1)
    {
      printf("ERROR %s/%s: v545_set_override_position_velocity() failure\n", whoami, __FUNCTION__);
      exit(1);
    }

  // begin main loop from min -> max -> min -> ...
  ...
  while (loop_flag)
    {
      ...
      while (write_position < max_position)
        {
          v545_write_synchro_resolver(pb, SYNCHRO_OUTPUT_FB_CHNL, min_position, max_position, write_position, signed_flag);
          
          usleep(usec_pause);

          v545_read_synchro_resolver(pb, SYNCHRO_INPUT_FB_CHNL, min_counts, max_counts, min_position, max_position, &read_position, &measured_secondary_voltage, signed_flag);
          v545_read_synchro_resolver_position_counts(pb, SYNCHRO_INPUT_FB_CHNL, &counts, signed_flag);
  ...
  if (strcmp(ans, "y") == 0)
    {
      ...
      if (v545_set_override_swtrigger(pb, OVERRIDE_BLOCK_CHNL) == -1)
        {
  ...
  if (v545_clear_function_block(pb, SYNCHRO_OUTPUT_FB_CHNL) == -1)
    {
